# KI-Prompts MDS edu-sharing 10.0
...
> **Stand der optimierten Fassungen:** Die „neu"-Prompts der Felder **Titel (2), Beschreibung (3), Schlagworte (4) und Materialart (5)** entsprechen dem Stand **`mds_28052026_v2.xml`** — d. h. die Rückmeldungen aus dem ersten Judge-Durchlauf wurden eingearbeitet. System-Prompt (1) und Themenseiten-Prompts (15) sind in V2 unverändert. Die Felder 6–12 sind in der `mds.xml` **nicht** als `aiConfig` verdrahtet; die hier gezeigten „neu"-Fassungen sind dokumentierte Optimierungsvorschläge und nicht Teil von `mds_28052026_v2.xml`.

## Optimierungsprinzipien

Die neuen Fassungen folgen den aktuellen Prompting-Empfehlungen von OpenAI (GPT-4.1) und Anthropic:

- **Explizit statt implizit.** LLM-Modelle befolgen Anweisungen wörtlicher als frühere Modelle; gewünschtes Format und Entscheidungsregeln werden klar benannt.
...
- **Globaler Config** (wie ursprünglich `suggestion_ai`) – `[system]` (Rolle, Regeln, Ausgabedisziplin) + `[user]` (Materialinformationen + Volltext). Statisch und cachebar (Abschnitt 1).
- **Feld-Auftrag** – die kurze, feldspezifische Anweisung (Abschnitte 2 ff.) wird – wie bei den alten Widget-Configs – an die `[user]`-Nachricht angehängt.
- **Themenseiten-Prompts** (Abschnitt 15) sind eigenständige Configs für ganze Sammlungen und nutzen einen anderen Datensatz (Sammlungsthema, Bildungsstufe, Fachgebiet, Schlagwörter).
...
**Neu (optimiert, Stand `mds_28052026_v2.xml`):**

```text
Formuliere einen suchoptimierten, inhaltsbeschreibenden Titel, der den INHALT präzise benennt – nicht die Form oder Zielgruppe.

- Länge: 40–70 Zeichen (Ausnahmen bei sehr kurzen Titeln sind möglich).
- Substantivisch formulieren, ohne Artikel und Sonderzeichen (keine Anführungszeichen, Doppelpunkte, Bindestriche o. Ä.).
- Materialtyp (z. B. Arbeitsblatt, Video, Quiz) und Zielgruppenangaben (z. B. für Schüler, Sekundarstufe) dürfen NICHT im Titel vorkommen.
- Nenne nicht nur das Fachgebiet (z. B. „Mathematik“), sondern das konkrete Thema oder die zentralen Inhalte.
- Platziere die wichtigsten Schlüsselbegriffe möglichst am Anfang des Titels.
- Verbinde Begriffe so, dass der Titel gut lesbar und verständlich bleibt – minimaler Einsatz von Bindewörtern ist erlaubt, sofern keine Artikel verwendet werden.
- Vermeide Füllwörter und irrelevante Zusatzinformationen.
- Beispiele:
  - „Fotosynthese Lichtreaktion Dunkelreaktion“
  - „Bruchrechnung Kürzen Erweitern“
  - „Kirche Mittelalter Macht Ablasshandel Glaubensvorstellungen“

Ausgabe: Nur der Titel, ohne weitere Erklärungen oder Zusätze.
```

**Geändert (V2, Judge-Feedback eingearbeitet):** zusätzlich explizites Verbot von Materialtyp- und Zielgruppenangaben; flexible Längenregel (Ausnahmen bei sehr kurzen Titeln); wichtigste Schlüsselbegriffe an den Anfang; minimaler Einsatz von Bindewörtern für bessere Lesbarkeit; erweiterte Beispiele.
...
```text
Erstelle eine motivierende und suchoptimierte Beschreibung des Inhalts mit folgenden Anforderungen:

- Länge: 400 bis 800 Zeichen.
- Satzlänge: Maximal 20 Wörter pro Satz.
- Sprache: Aktiv, klar, motivierend und zielgruppengerecht formuliert.
- Struktur (Reihenfolge strikt einhalten):
  1. Kernthema des Materials
  2. Zentrale Inhalte und Konzepte
  3. Didaktische Form (z. B. Methoden, Lernformate)
  4. Zielgruppe und Einsatzmöglichkeiten
  5. Mehrwert und Nutzen für Lernende
- Fachbegriffe und gängige Suchbegriffe sollen möglichst vielfältig, natürlich und sinnvoll eingebunden werden, um die Auffindbarkeit zu verbessern.
- Verwende ausschließlich Inhalte, die im Ausgangsmaterial belegt sind. Keine erfundenen Details, keine Ergänzungen ohne Beleg.
- Vermeide Floskeln wie "Dieses Material …", "Hier finden Sie …" sowie URLs, Dateinamen oder andere technische Angaben.
- Ausgabe: Nur der reine Beschreibungstext, ohne zusätzliche Formatierungen, Überschriften oder Felder.
```

**Geändert (V2, Judge-Feedback eingearbeitet):** verbindliche Satz-/Zeichenlängen; strikte 5-Punkt-Struktur; ausschließlich belegte Inhalte (keine Erfindungen); vielfältige, natürliche Einbindung von Such-/Fachbegriffen; verpflichtende Nennung von Zielgruppe und didaktischer Form; reiner Textoutput ohne Formatierung.
...
```text
Erzeuge 5 bis 8 inhaltliche Schlagworte zum Thema, sortiert nach absteigender Relevanz (Kernthema zuerst).

Beachte dabei folgende Vorgaben:

- Verwende ausschließlich Substantive im Nominativ Singular.
- Nutze etablierte Fachterminologie, die spezifisch und präzise ist (z. B. "Bruchrechnung" statt "Mathematik").
- Vermeide Synonyme, Dopplungen sowie die gleichzeitige Nennung von Ober- und Unterbegriffen.
- Schließe Materialtypen, Bildungsstufen, Markennamen, Personen (außer bei historischen oder literarischen Themen), Orte und irrelevante Kategorien aus.
- Gib die Schlagworte als einfache, kommagetrennte Liste ohne zusätzliche Formatierungen aus.
- Beispiel: "Bruchrechnung, Dezimalzahlen, Prozentrechnung, Grundrechenarten, Mathematikunterricht"
```

**Geändert (V2, Judge-Feedback eingearbeitet):** unmissverständliches Ausgabeformat (kommagetrennt, kein JSON, keine Aufzählung); Nominativ Singular für Konsistenz; klarere Ausschlüsse; Beispiel mit korrekter Formatierung ergänzt.
...
```text
Bestimme die Materialart ausschließlich nach der DIDAKTISCHEN FUNKTION, nicht nach dem Dateiformat.

Leitfrage: Was ist der hauptsächliche Zweck des Materials?
Beispiele für didaktische Funktionen:
- lesen/ansehen
- bearbeiten/lösen
- nachschlagen
- ausprobieren
...
Wähle genau EINE Kategorie, die die dominante didaktische Funktion des Materials beschreibt.
Bei Mischformen entscheide dich für die vorherrschende Funktion.

Folgende Kategorien stehen zur Auswahl (Name → id):
{{widget("ccm:educationallearningresourcetype".values[*].{id:id, name:caption})}}

Wichtig:
- Gib ausschließlich die id der gewählten Kategorie aus.
- Die id ist ein String ohne Anführungszeichen, ohne weitere Zusätze oder Erklärungen.
- Keine Ausgabe von Namen, JSON-Objekten oder sonstigen Kommentaren.
- Beispiel für korrekte Ausgabe: unterricht_planen

Ausgabe:
```

**Geändert (V2, Judge-Feedback eingearbeitet):** strikte „nur die id"-Ausgabe (kein Name, kein JSON, keine Anführungszeichen); id explizit als String; Leitfrage als hauptsächlicher Zweck präzisiert; Beispielausgabe ergänzt.
...
> Nicht in `mds_28052026_v2.xml` enthalten (Feld in der `mds.xml` ohne `aiConfig`). Untenstehende „neu"-Fassung ist ein dokumentierter Optimierungsvorschlag.
...
> Nicht in `mds_28052026_v2.xml` enthalten (Feld in der `mds.xml` ohne `aiConfig`).
...
> In `mds_28052026_v2.xml` unverändert gegenüber V1.
...
**Stand der Prompt-Sets:**

- `mds.xml` – IST-Stand (edu-sharing 10.0).
- `mds_28052026.xml` – optimierte Fassung V1 (manuell).
- `mds_28052026_v2.xml` – V2: Judge-Feedback aus Durchlauf 1 in Titel (2), Beschreibung (3), Schlagworte (4) und Materialart (5) eingearbeitet. System (1) und Themenseiten (15) unverändert.
...
- [ ] Felder 6–12 (Inhaltstyp, Bildungsstufe, Zielgruppe, Berufsgruppe, Textkomplexität, Sprache, Lernzeit): bei Bedarf als `aiConfig` in der `mds.xml` verdrahten, damit die optimierten Prompts produktiv greifen.
